...
class Map {
    ...
    drawMap(world) {
		...
		//let mapsvg =  mapdiv.select('#worldmap');
		let path = d3.geoPath()
		...
		d3.select('#map-chart').append('svg').attr("class" , 'worldmap').attr("width", 400).attr("height" , 400)
		        .selectAll("path")
				.data(countryData)
				.join("path")
				.attr("d", path)
		...
		//d3.select('#worldmap').selectAll("path")
         ...
         d3.select('.worldmap').append("path")
        .datum(graticule).attr('class', "graticule").attr('d',path);
		d3.select('.worldmap').append("path")
		    .datum(graticule.outline)
    .attr("class", "graticuleb")
    .attr("d", path);
